...
for i in para:
    i = i.strip()
    if i != "":
        # print(f"{Colors.GREEN}Alpha: {i}{Colors.ENDC}" if i.isalnum() else f"{Colors.RED}Not Alpha: {i}{Colors.ENDC}")
        trie.insert(i) 
...
test_cases = [
    ...
    ("useless","Not Found"),
    ("week","Found: 1"),
    ("architecture","Found: 1"),
]

def code_runner(para:str, test_cases:list):
    count = 0
    for test_in, test_out in test_cases:
        count+=1
        result = trie.search(test_in)
        print(f"Test Case {count}:", f"{Colors.GREEN}PASSED{Colors.ENDC}" if result ==test_out else f"{Colors.RED}FAILED{Colors.ENDC}" )
    return

code_runner(para, test_cases)